...
Core.Functions.CreateCallback('core-weapons:server:RepairWeapon', function(src, cb, RepairPoint, data)
    ...
    if not exports['qb-inventory']:RemoveItem(src, data.name, 1, data.slot, 'core-weapons:server:RepairWeapon') then
        Player.Functions.AddMoney('cash', Shared.Weapons.WeaponRepairCosts[WeaponClass], 'core-weapons:server:RepairWeapon')
        return
    end

    TriggerClientEvent('qb-inventory:client:ItemBox', src, Core.Shared.Items[data.name], 'remove')
    TriggerClientEvent('qb-inventory:client:CheckWeapon', src, data.name)
    TriggerClientEvent('core-weapons:client:SyncRepairShops', -1, Shared.Weapons.WeaponRepairPoints[RepairPoint], RepairPoint)
...
RegisterNetEvent('core-weapons:server:TakeBackWeapon', function(k)
    ...
    exports['qb-inventory']:AddItem(src, itemdata.name, 1, false, itemdata.info, 'core-weapons:server:TakeBackWeapon')
    TriggerClientEvent('qb-inventory:client:ItemBox', src, Core.Shared.Items[itemdata.name], 'add')
...
local function EquipWeaponAttachment(src, item)
    ...
    if shouldRemove then
        exports['qb-inventory']:RemoveItem(src, item, 1, false, 'core-weapons:EquipWeaponAttachment')
        TriggerClientEvent('qb-inventory:client:ItemBox', src, Core.Shared.Items[item], 'remove')
    end
end
    ...
    if Inventory[WeaponData.slot] then
        if Inventory[WeaponData.slot].info.attachments and next(Inventory[WeaponData.slot].info.attachments) then
            ...
            if HasAttach then
                ...
                Player.Functions.SetInventory(Player.PlayerData.items, true)
                exports['qb-inventory']:AddItem(src, AttachmentData.attachment, 1, false, false, 'core-weapons:server:RemoveAttachment')
                TriggerClientEvent('qb-inventory:client:ItemBox', src, Core.Shared.Items[AttachmentData.attachment], 'add')
                TriggerClientEvent('Core:Notify', src, Lang:t('info.removed_attachment', { value = Core.Shared.Items[AttachmentData.attachment].label }), 'error')
